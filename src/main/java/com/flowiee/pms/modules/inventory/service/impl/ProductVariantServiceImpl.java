...
public class ProductVariantServiceImpl extends BaseService<ProductDetail, ProductVariantDTO, ProductDetailRepository> implements ProductVariantService {
    ...
    public ProductVariantDTO save(ProductVariantDTO pDto) {
        try {
            ...
            boolean productVariantExists = isProductVariantExists(lvProduct.getId(), vldModel.getColor().getId(), vldModel.getSize().getId(), vldModel.getFabricType().getId());
            if (productVariantExists) {
                throw new BadRequestException("This product variant already exists!");
            }
