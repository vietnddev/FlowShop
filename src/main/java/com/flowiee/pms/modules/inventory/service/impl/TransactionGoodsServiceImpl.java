...
public class TransactionGoodsServiceImpl extends BaseService<TransactionGoods, TransactionGoodsDTO, TransactionGoodsRepository> implements TransactionGoodsService {
    ...
    public void restockReturnedItems(Long pStorageId, String pOrderCode) {
        ...
        TransactionGoods lvTransactionGoodsExport = lvOrder.getTransactionGoodsExport();
        for (TransactionGoodsItem lvItem : lvTransactionGoodsExport.getItems()) {
    ...
    @Override
    public List<FileStorage> getUploadedImagesImport(Long pTranId) {
        return fileStorageRepository.findByTransactionGoodsImportId(pTranId);
    }
    ...
    @Transactional
    @Override
    public TransactionGoodsDTO createExportTransactionFromOrder(TransactionGoodsDTO pTransactionGoodsDto, String pOrderCode) {
        Order lvOrder = orderRepository.findByOrderCode(pOrderCode);
        if (lvOrder == null) {
            throw new BadRequestException("Invaild order!");
        }
        if (lvOrder.getTransactionGoodsExport() != null) {
            throw new BadRequestException("Đơn hàng này đã được tạo phiếu xuất kho!");
        }

        pTransactionGoodsDto.setWarehouse(new StorageDTO(pTransactionGoodsDto.getWarehouse().getId()));
        pTransactionGoodsDto.setDescription("Exporting goods for order " + lvOrder.getCode());
        TransactionGoodsDTO lvExportTransactionDto = this.createExportTransaction(pTransactionGoodsDto);

        List<OrderDetail> lvOrderItems = lvOrder.getListOrderDetail();
        for (OrderDetail lvItem : lvOrderItems) {
            TransactionGoods lvExportTransactionEnt = TransactionGoods.builder().build();
            lvExportTransactionEnt.setId(lvExportTransactionDto.getId());

            transactionGoodsItemRepository.save(TransactionGoodsItem.builder()
                    .transactionGoods(lvExportTransactionEnt)
                    .productVariant(lvItem.getProductDetail())
                    .quantity(lvItem.getQuantity())
                    .build());
        }

        orderRepository.updateTicketExportInfo(lvOrder.getId(), lvExportTransactionDto.getId());

        return lvExportTransactionDto;
    }
    ...
    public String deleteExportTransaction(Long pTranId) {
        ...
        //Validate more
        Order order = orderRepository.findByTicketExport(lvTransactionGoods.getId());
        if (order != null) {
            order.setTransactionGoodsExport(null);
