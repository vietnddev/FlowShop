...
public class OrderServiceImpl extends BaseService<Order, OrderDTO, OrderRepository> implements OrderService {
    ...
    public OrderDTO createOrder(CreateOrderReq pRequest) {
        ...
        BigDecimal lvDefaultPackagingCost = BigDecimal.ZERO;
        BigDecimal lvDefaultShippingCost = BigDecimal.ZERO;
        BigDecimal lvDefaultGiftWrapCost = BigDecimal.ZERO;
        BigDecimal lvDefaultCodFee = BigDecimal.ZERO;
        ...
        Order order = Order.builder()
                ...
                .packagingCost(CoreUtils.coalesce(pRequest.getPackagingCost(), lvDefaultPackagingCost))
                .shippingCost(CoreUtils.coalesce(pRequest.getShippingCost(), lvDefaultShippingCost))
                .deliveryMethod(new Category(pRequest.getDeliveryMethodId()))
                .deliveredBy("STORE_DELIVERY")
                .giftWrapCost(CoreUtils.coalesce(pRequest.getGiftWrapCost(), lvDefaultGiftWrapCost))
                .codFee(CoreUtils.coalesce(pRequest.getCodFee(), lvDefaultCodFee))
                .isGiftWrapped(false)
        ...
        mvSystemLogService.writeLogCreate(ACTION.PRO_ORD_C, MasterObject.Order, "Thêm mới đơn hàng", lvOrderSaved.getCode());
    ...
    private LocalDateTime getOrderTime(String pOrderTime) {
        if (CoreUtils.isNullStr(pOrderTime)) {
    ...
    public OrderDTO updateOrder(UpdateOrderReq pRequest, Long pOrderId) {
        ...
        mvSystemLogService.writeLogUpdate(ACTION.PRO_ORD_U, MasterObject.Order, "Cập nhật đơn hàng", changeLog);
    ...
    public String deleteOrder(Long id) {
        ...
        mvSystemLogService.writeLogDelete(ACTION.PRO_ORD_D, MasterObject.Order, "Xóa đơn hàng", lvOrder.toString());
    ...
    private void executeRefundOrder(Order pOrder, ChangeOrderStatusReq pRequest) {
        ...
        mvSystemLogService.writeLogUpdate(ACTION.PRO_ORD_U, MasterObject.Order, "Trả đơn hàng " + lvOrder.getCode(),
                lvMessageLog.toString(), null);
    }
